...
export default class EscortMissionController {
  ...
  start() {
    ...
    window.__escortEmitter__.on(EVENTS.ACCEPT_ESCORT, (n: number) => {
      if (window.xself.isInTeam() && window.xself.isMember()) {
        const e = window.MsgHandler.createPlayerEvent(
          n,
  ...
  async controller() {
    ...
    if (!window.GameWorld.getEscort().isMoveTime())
      return

    if (!(window.GameWorld.getEscort().nextMoveTime < Date.now()))
      return
  ...
  private handleEvent(list: any[]) {
    ...
    if (this.position >= this.paths.length)
      return
