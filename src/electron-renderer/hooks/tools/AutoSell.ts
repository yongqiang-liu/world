...
export default class AutoSell {
  ...
  ai_sell() {
    ...
    const t = []
    const e = window.xself.getLevel()
    const n = (10 * Math.floor((2 * e) / 10)) / 2
    const i = window.xself.bag
    if (i == null)
      return
    for (let o = window.PlayerBag.BAG_START; o <= i.bagEnd; o++) {
      const a = i.store[o]
      // 该背包格为空
      if (a == null)
        continue
      ...
      // 不出售钥匙、钻石
      if (a.name.includes('钥匙') || a.name.includes('钻石'))
        continue

      // 不出售坐骑
      if (a.type == Define.ITEM_TYPE_ARMOR_TRANSPORT)
        continue
      // 不出售时装
